<template>
	<div class="hotelupload" v-loading="loading2" element-loading-text="操作进行中" element-loadig-spinner="el-icon-loading" element-loading-background="rgba(0,0,0,.4)">
		<!-- <p class="head">
			...
			<span class="middle">酒店管理</span>
			<span class="middle">></span>
			<span class="middle">酒店相册</span>
		</p> -->
		<div class="contain">
...
import { mapGetters } from 'vuex'
    ...
    methods: {
        ...
        API.put("/pms/img/logo",obj).then((res=>{
          // console.log('nnnnnnnnn0000000',res)
          if(res.error_code == 0) {
    	...
    	getLoadedImg() {
    		return API.get("/pms/img?id="+this.hotel.id+"&type="+this.hotelPhotosType)
    	},
    	...
    	gettoken() {
    		return API.hotelUploadGetToken({id: this.hotel.id, type: this.hotelPhotosType})  //   gai```````````````
    	},
