...
let hosts = "https://test.yunsuit.com";//外网测试环境
let api = {
  // 更新用户信息
  "userMsg":'/api/v1/user',
  // 登录
...
// 请求数据
let request = function ({ url, data, method, header = { 'Content-Type': 'application/json', 'Token': wx.getStorageSync("token") } }) {
  wx.showToast({
    title: '加载中...',
    icon: 'loading'
  });
...
// 未支付订单重新支付下单
let repeatOrder = function ({ openUrl, id, pay_way}) {
  if (!pay_way) {
    wx.showToast({
      title: '请选择支付方式',
      icon: "none"
